...
class Conversation(Base):
    ...
    learning_mode_enabled = Column(Boolean, default=False)  # Track learning mode preference
    user_settings = Column(JSON, nullable=True)  # Store user preferences (safety level, transparency, etc.)
...
class Message(Base):
    ...
    priority_level = Column(String, nullable=True, index=True)  # critical, high, medium, low
    escalation_reason = Column(Text, nullable=True)  # Why it was escalated
    target_response_time = Column(Integer, nullable=True)  # Target response time in minutes
    
    # Relationships
    conversation = relationship("Conversation", back_populates="messages")
    moderator_decisions = relationship("ModeratorDecision", back_populates="message", cascade="all, delete-orphan")
...
class ModeratorDecision(Base):
    ...
    moderator_id = Column(Integer, ForeignKey("users.id"), nullable=True)  # Null for anonymous moderators
    ...
    if 'messages' in inspector.get_table_names():
        ...
        with engine.connect() as conn:
            if 'confidence_score' not in existing_columns:
                conn.execute(text("ALTER TABLE messages ADD COLUMN confidence_score REAL"))
                conn.commit()
                print("✅ Added confidence_score column to messages table")
            
            if 'confidence_level' not in existing_columns:
                conn.execute(text("ALTER TABLE messages ADD COLUMN confidence_level VARCHAR"))
                conn.commit()
            ...
            if 'pii_detected' not in existing_columns:
                conn.execute(text("ALTER TABLE messages ADD COLUMN pii_detected BOOLEAN DEFAULT 0"))
                conn.commit()
