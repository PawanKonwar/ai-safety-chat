...
from auth import (
    ...
    verify_password, get_or_create_anonymous_session
)
...
SAFETY_KEYWORDS = {
    ...
    "crisis": [
        "suicide", "suicidal", "depressed", "kill myself", "end my life", "hopeless",
        "i want to die", "want to die", "don't want to live", "better off dead",
        "going to kill", "end it all", "no reason to live", "hurt myself",
        "self harm", "cutting myself", "want to end", "not worth living"
    ]
}
...
class MockOpenAI:
    ...
    @staticmethod
    def generate_response(user_message: str, category: Optional[str] = None, pii_types: Optional[List[str]] = None) -> str:
        ...
        crisis_keywords_direct = [
            "i want to die", "want to die", "kill myself", "end my life", "suicide", "suicidal",
            "don't want to live", "better off dead", "going to kill", "end it all", 
            "no reason to live", "hurt myself", "self harm", "cutting myself", "not worth living"
        ]
        ...
        elif "2+2" in lower_message or "what is 2+2" in lower_message or lower_message.strip() == "2+2" or "2 + 2" in lower_message:
            return "2 + 2 equals 4. This is a basic mathematical fact with 100% certainty."
        elif "3*3" in lower_message or "3 * 3" in lower_message or "3 times 3" in lower_message or "what is 3*3" in lower_message:
            return "3 times 3 equals 9. This is a basic mathematical fact with 100% certainty."
        elif "10-5" in lower_message or "10 - 5" in lower_message or "what is 10-5" in lower_message:
            return "10 minus 5 equals 5. This is a basic mathematical fact with 100% certainty."
        elif ("capital" in lower_message and "france" in lower_message) or lower_message.strip() == "capital of france" or "france capital" in lower_message:
            return "Paris is the capital of France. This is a well-established geographical fact with 100% certainty."
        elif ("capital" in lower_message and "japan" in lower_message) or "japan capital" in lower_message:
            return "Tokyo is the capital of Japan. This is a well-established geographical fact with 100% certainty."
        elif "water boils" in lower_message or "boiling point of water" in lower_message:
        ...
        elif "earth orbits" in lower_message or "earth revolves around sun" in lower_message:
            return "Earth orbits the Sun. This is a well-established astronomical fact with 100% certainty."
        elif "photosynthesis" in lower_message and ("explain" in lower_message or "what is" in lower_message):
        ...
        elif "invest" in lower_message or ("stock" in lower_message and "should" in lower_message):
        ...
        elif "will" in lower_message and ("ai" in lower_message or "job" in lower_message):
        ...
        elif "what is" in lower_message or "explain" in lower_message or "define" in lower_message:
...
class ContextAnalysis(BaseModel):
    ...
    previous_queries: List[Dict[str, Any]]  # Recent queries with categories (confidence can be float, str, or None)
...
def calculate_priority(category: Optional[str], confidence: float, content: str, confidence_score: float = 0.0) -> tuple[str, str, int]:
    """
    ...
    crisis_keywords = [
        "i want to die", "kill myself", "end my life", "suicide", "suicidal",
        "want to die", "going to kill", "end it all", "no reason to live",
        "better off dead", "hurt myself", "self harm", "cutting myself",
        "don't want to live", "not worth living", "want to end"
    ]
    ...
    if category == "legal":
        ...
        illegal_keywords = ["how to", "help me", "can you help", "instructions", "guide", "tutorial"]
        has_illegal_intent = any(keyword in lower_content for keyword in illegal_keywords)
    ...
    if category == "financial":
        ...
        high_risk_financial = ["invest all", "borrow money", "take loan", "credit card debt", "gambling"]
    ...
    controversial_keywords = ["holocaust", "genocide", "war crimes", "atrocity", "massacre"]
    ...
    political_keywords = ["president", "election", "political party", "voting", "campaign", "politician"]
    ...
    religious_keywords = ["god", "religion", "faith", "prayer", "church", "temple", "mosque", "bible", "quran"]
...
def check_safety_filter(message: str):
    ...
    for keyword in crisis_keywords:
        if keyword.lower() in lower_message:
            ...
            if "i want to die" in keyword.lower() or "kill myself" in keyword.lower() or "suicide" in keyword.lower():
    ...
    if detected_categories:
        ...
        keyword_count = sum(1 for keyword in SAFETY_KEYWORDS[category] 
                          if keyword.lower() in lower_message)
...
def detect_and_redact_pii(text: str) -> tuple[str, List[str], str]:
    ...
    phone_context_pattern = r'\b(?:phone|call|text|contact|number|tel|mobile)\s*[:\-]?\s*\d{10}\b'
    ...
    if re.search(address_pattern, text, re.IGNORECASE):
        redacted_text = re.sub(address_pattern, '[REDACTED]', redacted_text, flags=re.IGNORECASE)
...
def calculate_confidence_score(user_message: str, ai_response: str, category: Optional[str] = None) -> tuple[float, str, List[str]]:
    """
    ...
    factual_patterns = [
        ...
        "capital of", "invented", "discovered", "created"
    ]
    
    # Subjective/opinion patterns
    subjective_patterns = [
        "should i", "what should i", "do you think", "do you recommend",
        "best", "worst", "better", "prefer", "favorite", "opinion",
        "think about", "believe", "feel", "like"
    ]
    
    # Personal advice patterns
    personal_advice_patterns = [
        "should i", "what should i do", "what should i", "advice",
        "recommend", "suggest", "tell me what to", "help me decide"
    ]
    
    # Future prediction patterns
    future_patterns = [
        "will", "going to", "predict", "forecast", "future",
        "tomorrow", "next year", "will happen", "will it"
    ]
    
    # Historical/factual indicators
    historical_patterns = [
        "invented", "discovered", "created", "founded", "established",
        "who invented", "who discovered", "when was", "when did"
    ]
    
    # Scientific/educational indicators
    scientific_patterns = [
        "science", "physics", "chemistry", "biology", "math", "mathematics",
        "photosynthesis", "gravity", "temperature", "boils at", "formula",
        "equation", "theory", "law of"
    ]
    ...
    is_personal_advice = any(pattern in lower_message for pattern in personal_advice_patterns)
    ...
    if is_factual and not is_subjective and not is_personal_advice:
        ...
        if any(op in lower_message for op in ["+", "-", "*", "×", "÷", "/", "times", "plus", "minus", "equals"]):
    ...
    # Check for LOW confidence (0-49%) - Personal advice, predictions, sensitive topics
        ...
        elif "invest" in lower_message or "buy" in lower_message or "stock" in lower_message:
            score = 30.0
        ...
        score = 15.0
        reasons.append("Crisis content requires immediate human intervention and professional support")
    elif category in ["medical", "financial", "legal"]:
        score = 30.0
        reasons.append(f"Topic involves {category} content requiring professional expertise")
    ...
    elif "today" in lower_message or "current" in lower_message or "recent" in lower_message:
        score = 60.0
    ...
    uncertain_language = ["maybe", "perhaps", "might", "could", "possibly", "uncertain", "unclear", "not sure"]
    ...
    factual_indicators = ["fact", "established", "research", "study", "data", "evidence", "scientific", "verifiable"]
    ...
    if is_factual and any(word in lower_response for word in ["equals", "is", "was", "are", "were"]):
        if score < 90:
...
def analyze_conversation_context(conversation_history: List[Dict], new_message: str, new_category: Optional[str] = None, confidence: float = 0.0) -> Dict:
    """
    ...
    if new_category in sensitive_categories:
        ...
        previous_same_category = [m for m in recent_user_messages if m.get("category") == new_category]
        ...
        if len(previous_same_category) > 0:
            ...
            if new_category == "medical":
                ...
                severity_keywords = {
                    ...
                    "high": ["chest pain", "difficulty breathing", "emergency", "urgent", "can't breathe", "heart", "stroke"]
                }
                
                # Check for escalation from low to high severity
                previous_content = " ".join([m["content"].lower() for m in previous_same_category])
                new_severity = "low"
                prev_severity = "low"
                ...
                if (prev_severity == "low" and new_severity in ["medium", "high"]) or \
                   (prev_severity == "medium" and new_severity == "high"):
                    ...
                    analysis["context_flags"].append("Medical risk escalation detected in conversation")
            
            # Financial escalation patterns
            elif new_category == "financial":
                # Check if moving from general to specific advice requests
                lower_new = new_message.lower()
                if any(term in lower_new for term in ["invest", "buy", "sell", "trade", "strategy"]):
                    if any("money" in m["content"].lower() or "earn" in m["content"].lower() 
                           for m in previous_same_category):
                        analysis["risk_escalation"] = True
                        analysis["context_flags"].append("Financial advice escalation detected")
        
        # Check for persistent sensitive queries
        if len(previous_same_category) >= 2:
            analysis["persistent_sensitive_topic"] = True
            analysis["context_flags"].append(f"Multiple {new_category} queries in conversation")
    ...
    if new_category is None and len(recent_user_messages) > 0:
        ...
        previous_sensitive = [m for m in recent_user_messages if m.get("category") in sensitive_categories]
        
        if previous_sensitive:
            # Check if new message is semantically similar but avoids keywords
            lower_new = new_message.lower()
            bypass_patterns = [
                ("medical", ["health", "body", "feel", "symptom", "doctor", "treatment"]),
                ("financial", ["money", "cash", "wealth", "income", "profit", "return"]),
                ...
                if any(m.get("category") == cat for m in previous_sensitive):
                    ...
                    if any(kw in lower_new for kw in keywords):
                        ...
                        for keyword in SAFETY_KEYWORDS.get(cat, []):
                            ...
                            analysis["context_flags"].append(f"Possible filter bypass attempt ({cat} topic)")
                            break
    ...
    if risk_scores:
        ...
        weights = [i + 1 for i in range(len(risk_scores))]  # More recent = higher weight
        analysis["cumulative_risk_score"] = sum(s * w for s, w in zip(risk_scores, weights)) / sum(weights)
...
def generate_learning_analysis(
    ...
    """
    analysis = {
        "risk_category": category.capitalize() if category and category != "safe" else "Safe",
    ...
    if pii_types and len(pii_types) > 0:
        ...
        analysis["safety_tips"].append("Personal information was automatically redacted for your privacy")
    ...
    if category == "medical":
        ...
        analysis["safety_tips"].append("Consult a healthcare professional for medical advice")
        analysis["human_review_reason"] = "Medical queries require professional oversight"
    elif category == "financial":
        analysis["safety_tips"].append("AI cannot access your financial situation")
        analysis["safety_tips"].append("Financial decisions should be made with professional guidance")
        analysis["human_review_reason"] = "Specific financial advice requires human oversight"
    elif category == "legal":
        analysis["safety_tips"].append("AI cannot provide legal representation")
        analysis["safety_tips"].append("Legal matters require consultation with a qualified attorney")
        analysis["human_review_reason"] = "Legal queries require professional legal review"
    elif category == "crisis":
        analysis["safety_tips"].append("If you're in crisis, please contact emergency services or a crisis hotline")
        analysis["human_review_reason"] = "Crisis content requires immediate human intervention"
    elif confidence_score >= 80:
        analysis["safety_tips"].append("This response has high confidence based on verifiable facts")
    elif confidence_score >= 50:
        analysis["safety_tips"].append("This response has moderate confidence - verify important information")
    else:
        analysis["safety_tips"].append("This response has low confidence - exercise caution and verify information")
    
    # Add AI limitations explanation
    if not analysis["safety_tips"]:
        analysis["safety_tips"].append("AI responses are based on training data and may not reflect current information")
    
    return analysis

# Generate AI Response
async def generate_ai_response(user_message: str, category: Optional[str] = None, pii_types: Optional[List[str]] = None) -> str:
    """
...
async def chat(
    ...
    if not conversation:
        conversation = Conversation(
            ...
            user_settings=settings.model_dump() if (settings and hasattr(settings, 'model_dump')) else (settings.dict() if settings else None)
        )
        db.add(conversation)
        db.commit()
        ...
        # Update preferences
        conversation.learning_mode_enabled = settings.learning_mode or request.learning_mode
        # Use model_dump() for Pydantic v2, fallback to dict() for v1
    ...
    if conversation:
        recent_messages = db.query(Message).filter(
            Message.conversation_id == conversation.id
        ).order_by(Message.timestamp.desc()).limit(9).all()
    ...
    if context_analysis.risk_escalation or context_analysis.filter_bypass_attempt or \
       context_analysis.persistent_sensitive_topic:
        context_flagged = True
        # Adjust confidence score based on cumulative risk
        if context_analysis.cumulative_risk_score > 0.6:
            confidence_score = max(0, confidence_score - 15)  # Lower confidence for high cumulative risk
    ...
    if final_flagged:
        ...
        if priority_level == "critical":
            print(f"🚨 Priority calculated: {priority_level.upper()} - {escalation_reason} (target: {target_response_time} min)")
    ...
    if store_messages:
        ...
        user_msg = Message(
            ...
            pii_types=pii_types if pii_types else None,  # SQLAlchemy JSON will serialize this
    ...
    if final_flagged:
        ...
        category_name = category_names.get(category, category) if category else "Low Confidence"
        ...
        if flag_reasons:
            message_for_moderator = f"Flagged for: {', '.join(flag_reasons)}. Message: {user_message[:100]}"
        else:
        ...
        try:
            ...
            try:
                ...
                context_dict = {
                    ...
                    "cumulative_risk_score": getattr(context_analysis, 'cumulative_risk_score', 0.0),
                    "persistent_sensitive_topic": getattr(context_analysis, 'persistent_sensitive_topic', False),
                    "context_flags": getattr(context_analysis, 'context_flags', []),
                    "previous_queries": getattr(context_analysis, 'previous_queries', [])
                }
...
async def get_moderator_queue(
    ...
    for user_msg in flagged_user_messages:
        ...
        has_decision = db.query(ModeratorDecision).filter(
            ModeratorDecision.message_id == user_msg.id
        ).first()
        ...
        priority_level = ai_msg.priority_level if ai_msg and ai_msg.priority_level else (user_msg.priority_level if user_msg.priority_level else None)
        escalation_reason = ai_msg.escalation_reason if ai_msg and ai_msg.escalation_reason else (user_msg.escalation_reason if user_msg.escalation_reason else None)
        target_response_time = ai_msg.target_response_time if ai_msg and ai_msg.target_response_time else (user_msg.target_response_time if user_msg.target_response_time else None)
...
async def remove_from_queue(
    ...
    decision = ModeratorDecision(
        ...
        moderator_id=current_user.id if current_user.role in ["moderator", "admin"] else None,
...
async def moderator_action(
    ...
    if action_request.action == "edit":
        ...
        if not action_request.alternative_response:
            raise HTTPException(status_code=400, detail="alternative_response is required for reject action")
    ...
    examples = [
        {
            ...
            "confidence_level": "High",
            "reasons": [
                "Query asks for verifiable geographical fact"
            ],
            "category": "safe"
        },
        {
        ...
        },
        {
            ...
            "confidence_level": "High",
            "reasons": [
                "Query asks for verifiable historical fact"
            ],
            "category": "safe"
        },
        {
            ...
            "confidence_level": "High",
            "reasons": [
                "Query asks for verifiable scientific fact"
            ],
            "category": "safe"
        },
        {
            ...
            "confidence_level": "Medium",
            "reasons": [
                "Query requests subjective comparison or opinion"
            ],
            "category": "safe"
        },
        {
            ...
            "confidence_level": "Low",
            "reasons": [
                "Query requests personal financial advice"
            ],
            "category": "financial"
        },
        {
            ...
            "confidence_level": "Low",
            "reasons": [
                "Query involves future predictions with uncertainty"
            ],
            "category": "safe"
        },
        {
            ...
            "confidence_level": "Medium",
            "reasons": [
                "Weather information requires current data"
            ],
            "category": "safe"
        }
    ]
